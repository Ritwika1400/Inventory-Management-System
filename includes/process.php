...
//to update/edit user
if(isset($_POST["user_name"]) AND isset($_POST["id"])){
    $m = new User();
    $id = $_POST["id"];
    $name = $_POST["user_name"];
    $result = $m->update_record("user",["id"=>$id],["username"=>$name]);
    echo $result;
}
		...
		foreach ($rows as $row) {
			?>
				<tr>
			        ...
			        <td>
			        	<a href="#" pid="<?php echo $row['invoice_no']; ?>" class="btn btn-info btn-sm print_order">Print</a>
			        </td>
			      </tr>
			<?php
			$n++;
